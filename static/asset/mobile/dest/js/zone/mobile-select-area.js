    ...
    MobileSelectArea.prototype = {
        ...
        bindEvent: function() {
            ...
            this.trigger[_this.settings.eventName](function(e) {
                ...
                layer.open({
                    type: 1,
                    content: '<div class="ui-scroller-mask"><div id="' + _this.id + '" class="ui-scroller">' + dlgContent + '<p></p></div></div>',
                    btn: ['确定', '取消'],
                    success: function() {
                        // 禁用鼠标滚轮滚动
                    ...
                    yes: function (index) {
                        ...
                        if(typeof mol === 'object'){
                            mol.filter('.J_filter_obj','.J_filter_elem','.J_filter_con',1);
                        }
                    }, no: function(){
                        // 解除touchmove绑定、启用滚动
                        $('body').off();
        ...
        submit: function() {
            ...
            if (this.trigger[0].nodeType == 1) {
                //input
                this.trigger.html(this.text.join(this.separator));
                if(this.text[0] == this.text[1]){
                    this.trigger.html(this.text[0]);
                }
                this.trigger.attr('data-value', this.value.join(','));
            }
            // this.trigger.next(':hidden').val(this.value[0]+ ',' +this.value[1]);
            this.trigger.next(':hidden').val(this.value[1]);
            this.settings.callback && this.settings.callback.call(this,this.scroller,this.text,this.value);
            this.trigger.removeClass("cl9");
        },
        cancel: function() {
